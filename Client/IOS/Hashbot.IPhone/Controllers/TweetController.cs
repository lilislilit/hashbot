...
namespace Hashbot.IPhone
{
	public partial class TweetController : UIViewController
	{
		...
		void InitLayout()
		{
			...
			var dateFont = Fonts.Helvetica(10);
			var dateString = (NSString)_tweet.CreatedAt.ToString("dd.MM.yyyy");
			var dateStringSize = dateString.StringSize(dateFont);
			_dateLabel = new UILabel(new RectangleF(20, _line.Frame.Bottom + 2,dateStringSize.Width, 25));
			...
			var urlFont = Fonts.Helvetica(10);
			var urlLabelWidth = _line.Frame.Width - _dateLabel.Frame.Width;
			var urlStartPosition = _line.Frame.Right - urlLabelWidth + _dateLabel.Frame.Right;
			_urlLabel = new UILabel(new RectangleF(urlStartPosition, _line.Frame.Bottom + 2,urlLabelWidth, 25));
			_urlLabel.Text = _tweet.Url;
			_urlLabel.Font = urlFont;
